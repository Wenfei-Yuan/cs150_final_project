...
class ReadingSession(Base):
    ...
    llm_suggested_mode: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)

    # Setup questionnaire answers (0-3 index)
    reading_purpose: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    marked_for_retry: Mapped[Optional[list]] = mapped_column(JSON, default=list)

    # Reading mainline: ordered list of chunk indices for current mode
    reading_order: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)

    # Track which chunks have been read (for jump-back in skim/goal modes)
    jump_return_index: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
